...
/// <summary>
/// Operator evolution timeline via composition hub and existing operational read services (no evolution/digest service recursion).
/// </summary>
public sealed class OperationalEvolutionService : IOperationalEvolutionService
{
    ...
    private readonly ILogger<OperationalEvolutionService> _logger;

    public OperationalEvolutionService(
        ...
        ILogger<OperationalEvolutionService> logger)
    {
    ...
    public async Task<OperationalEvolutionTimelineDto> GetEvolutionTimelineAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadEvolutionContextAsync(cancellationToken).ConfigureAwait(false);
        var priorEvolutionSnapshots = _evolutionSnapshotStore.GetSnapshots();
        ...
        var timeline = OperationalEvolutionAggregation.ComposeEvolutionTimeline(
            ...
            context.DigestSnapshots,
            context.IntegritySnapshots,
            context.SituationSnapshots,
            context.PatternSnapshots,
            context.ExperienceSnapshots,
            context.SimulationSnapshots,
            priorEvolutionSnapshots,
            generatedAtUtc);

        _evolutionSnapshotStore.Append(OperationalEvolutionAggregation.CreateSnapshot(timeline));

        _logger.LogInformation(
            "Operational evolution observability: timeline composed. Direction={Direction}, TransitionCount={TransitionCount}",
            timeline.DominantEvolutionDirection,
            timeline.ActiveTransitionCount);

        return timeline;
    }

    public async Task<OperationalEvolutionSummaryDto> GetEvolutionSummaryAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadEvolutionContextAsync(cancellationToken).ConfigureAwait(false);
        var generatedAtUtc = DateTime.UtcNow;
        ...
        var momentum = OperationalEvolutionAggregation.ComposeMomentumAnalysis(
            context.Recovery,
            context.SituationRoom,
            context.IntegrityReport,
            context.Digest,
        ...
        var summary = OperationalEvolutionAggregation.ComposeEvolutionSummary(
            timeline,
            context.Recovery,
            context.SituationRoom,
            momentum,
        ...
        _logger.LogInformation(
            "Operational evolution observability: summary composed. MomentumState={MomentumState}",
            summary.OperationalMomentumState);

        return summary;
    }

    public async Task<OperationalMomentumAnalysisDto> GetMomentumAnalysisAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadEvolutionContextAsync(cancellationToken).ConfigureAwait(false);
        var generatedAtUtc = DateTime.UtcNow;
        ...
        _logger.LogInformation(
            "Operational evolution observability: momentum composed. Recovery={RecoveryMomentum}, Escalation={EscalationMomentum}",
            momentum.RecoveryMomentum,
            momentum.EscalationMomentum);

        return momentum;
    }

    private async Task<OperationalEvolutionContext> LoadEvolutionContextAsync(CancellationToken cancellationToken)
    {
        ...
        return new OperationalEvolutionContext
        {
            Trend = trend,
            Recovery = recovery,
            ...
            DigestSnapshots = _digestSnapshotStore.GetSnapshots(),
            IntegritySnapshots = _integritySnapshotStore.GetSnapshots(),
            SituationSnapshots = _situationSnapshotStore.GetSnapshots(),
            PatternSnapshots = _patternSnapshotStore.GetSnapshots(),
            ExperienceSnapshots = _experienceSnapshotStore.GetSnapshots(),
            SimulationSnapshots = _simulationSnapshotStore.GetSnapshots()
        };
    }

    private sealed class OperationalEvolutionContext
    {
        ...
        public IReadOnlyList<OperationalDigestSnapshot> DigestSnapshots { get; init; } = Array.Empty<OperationalDigestSnapshot>();
        public IReadOnlyList<OperationalIntegritySnapshot> IntegritySnapshots { get; init; } = Array.Empty<OperationalIntegritySnapshot>();
        public IReadOnlyList<OperationalSituationSnapshot> SituationSnapshots { get; init; } = Array.Empty<OperationalSituationSnapshot>();
        public IReadOnlyList<OperationalPatternSnapshot> PatternSnapshots { get; init; } = Array.Empty<OperationalPatternSnapshot>();
        public IReadOnlyList<OperationalExperienceSnapshot> ExperienceSnapshots { get; init; } = Array.Empty<OperationalExperienceSnapshot>();
        public IReadOnlyList<OperationalSimulationSnapshot> SimulationSnapshots { get; init; } = Array.Empty<OperationalSimulationSnapshot>();
    }
}
