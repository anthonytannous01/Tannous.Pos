...
/// <summary>
/// Operator resilience cognition via composition hub and existing operational read services (no resilience/convergence service recursion).
/// </summary>
public sealed class OperationalResilienceCognitionService : IOperationalResilienceCognitionService
{
    ...
    private readonly IOperationalResilienceCognitionSnapshotStore _resilienceSnapshotStore;
    private readonly ILogger<OperationalResilienceCognitionService> _logger;

    public OperationalResilienceCognitionService(
        ...
        IOperationalResilienceCognitionSnapshotStore resilienceSnapshotStore,
        ILogger<OperationalResilienceCognitionService> logger)
    {
        ...
        _resilienceSnapshotStore = resilienceSnapshotStore;
        _logger = logger;
    }

    public async Task<OperationalResilienceReportDto> GetResilienceReportAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadResilienceContextAsync(cancellationToken).ConfigureAwait(false);
        var priorSnapshots = _resilienceSnapshotStore.GetSnapshots();
        ...
        var report = OperationalResilienceAggregation.ComposeResilienceReport(
            ...
            context.ProtectiveContainmentActive,
            priorSnapshots,
            generatedAtUtc);
        ...
        _resilienceSnapshotStore.Append(OperationalResilienceAggregation.CreateSnapshot(report, fragilities));

        _logger.LogInformation(
            "Operational resilience observability: report composed. SurvivabilityState={SurvivabilityState}, FragilityCount={FragilityCount}",
            report.SurvivabilityState,
            fragilities.Count);

        return report;
    }

    public async Task<OperationalResiliencePostureSummaryDto> GetResilienceSummaryAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadResilienceContextAsync(cancellationToken).ConfigureAwait(false);
        var generatedAtUtc = DateTime.UtcNow;
        ...
        var report = OperationalResilienceAggregation.ComposeResilienceReport(
            ...
            context.ConvergenceReport,
            context.ProtectiveContainmentActive,
            _resilienceSnapshotStore.GetSnapshots(),
            generatedAtUtc);
        ...
        var summary = OperationalResilienceAggregation.ComposeResilienceSummary(
            report,
            context.SituationRoom,
            fragilities,
            generatedAtUtc);
        ...
        _logger.LogInformation(
            "Operational resilience observability: posture summary composed. SurvivabilityState={SurvivabilityState}",
            summary.OperationalSurvivabilityState);

        return summary;
    }

    public async Task<IReadOnlyList<OperationalFragilityDto>> GetOperationalFragilityAsync(
        CancellationToken cancellationToken = default)
    {
        var context = await LoadResilienceContextAsync(cancellationToken).ConfigureAwait(false);
        ...
        _logger.LogInformation(
            "Operational resilience observability: fragility composed. FragilityCount={FragilityCount}",
            fragilities.Count);

        return fragilities;
    }

    private async Task<OperationalResilienceContext> LoadResilienceContextAsync(CancellationToken cancellationToken)
    {
        ...
        return new OperationalResilienceContext
        {
            Recovery = recovery,
            ...
            ProtectiveContainmentActive = protectiveModeActive || runtimeSignals.ProtectiveContainmentActive
        };
    }
    ...
    private static string NormalizeArea(string area)
    {
        return string.IsNullOrWhiteSpace(area)
            ? OperationalResilienceAggregation.AreaOperational
            : area.Trim();
    }

    private sealed class OperationalResilienceContext
    {
